...
Each runner-up above names the scale at which it becomes the right call, so these migration paths are concrete.
...
- **Chat harness** (`klerk-cli` TS + Pi runtime, v4 stack table) → replaced by the
  Textual + textual-serve dual surface (v7-1); the Pi/TS experiments are archived (superseded by the Textual surface; kept in git history).
